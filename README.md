...
## Usage
- item Place raw data files (see below) in the 'Data/' folder
- item Run preprocess.py to generate clean parquet files (see below)
- item Run (and/or edit) travail.py to compute ion formation rate and generate plots

## Data files
This project has been made with csv data file as inputs. The csv were made as follow:
- item column headers: time,$X_1$,$X_2$,...,$X_i$ ($X_i$ beeing the particle size bins with $X_1$ the smallest, in nanometer)
- item time column: yyyy-mm-dd hh:mm:ss ('latin-1')
- particle size columns: dndlogdp to concentration
